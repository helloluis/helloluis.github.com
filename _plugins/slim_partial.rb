...
module Jekyll
  ...
  class SlimPartialTag < Liquid::Tag
    ...
    def set_slim_options(context)
      begin
        # Set a slim option in your config.yml, then pretty: true to have # pretty html output.
        Slim::Engine.default_options[:pretty] = context.registers[:site].config["slim"]["pretty"]
      rescue
      end
    end
